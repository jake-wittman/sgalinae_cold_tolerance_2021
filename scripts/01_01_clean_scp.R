...
# Survival data - convert data into a long data frame for survival format
survival_dat <- dat %>% 
   select(uid,
          date_SC,
          trial, 
          ...
          discolor_7day,
          est_scp) %>% 
   gather(day_discolor, discolor, discolor_3day, discolor_7day)

survival_dat <- dat %>% 
   select(uid,
          movement_3day,
          movement_7day) %>% 
   gather(day_movement, movement, movement_3day, movement_7day) %>% 
   mutate(day = case_when(
      day_movement == "movement_3day" ~ 3,
      day_movement == "movement_7day" ~ 7)) %>% 
   inner_join(survival_dat, by = "uid")
